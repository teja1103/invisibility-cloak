...
background_frames = []
for i in range(100):  # Increase the number of frames captured
...
while cap.isOpened():
    ...
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, np.ones((5, 5), np.uint8))  # Increase kernel size
    mask = cv2.morphologyEx(mask, cv2.MORPH_DILATE, np.ones((7, 7), np.uint8))  # Increase kernel size
    mask = cv2.GaussianBlur(mask, (21, 21), 0)  # Adjust GaussianBlur parameters
